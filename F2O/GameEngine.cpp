...
GameEngine::GameEngine(ContextSettings settings) :
    ...
    _walls(5), _coins(3), _backgroundSpeed(BACKGROUND_SPEED)
{}
...
bool    GameEngine::_gameOver()
{
    ...
    over.setStyle(Text::StrikeThrough);
    ...
    _showGameOver(game, over);
    return _playAgain(game, over);
}

void    GameEngine::_showGameOver(Text &game, Text &over)
{
    unsigned int cpt = 1;
...
bool    GameEngine::_playAgain(Text &game, Text &over)
{
    Event event;
    bool quit = false;
    ...
    while (_window.waitEvent(event))
    {
        ...
        if (event.type == Event::KeyPressed)
        {
            ...
            {
                quit = !quit;
                game.setStyle(quit ? Text::StrikeThrough : Text::Bold
                              );
                over.setStyle(quit ? Text::Bold : Text::StrikeThrough);
                _window.clear();
                _draw();
                _window.draw(game);
                _window.draw(over);
...
void    GameEngine::_moveBackground(const unsigned short id)
{
    ...
    if (_background[id].getPosition().x < 0 && _background[!id].getPosition().x < 0 &&
