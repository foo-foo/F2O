...
{
    Keyboard::Key   keysPressed[3] = {Keyboard::A, Keyboard::D, Keyboard::Space};
    ...
    if (event.type == Event::KeyReleased &&
        ...
        _releaseJump();
    if (event.type == Event::KeyReleased &&
        ((event.key.code >= Keyboard::A && _moving == LEFT) ||
         (event.key.code >= Keyboard::D && _moving == RIGHT)))
        _moving = NONE;
}

CircleShape const   &Character::getSprite() const
{
    return _sprite;
}
